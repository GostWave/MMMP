...
def control_gripper(angle):
    if main_gripper_joint != -1:
        p.setJointMotorControl2(aubo, main_gripper_joint, p.POSITION_CONTROL, targetPosition=angle, force=10)
    for joint_idx, multiplier in mimic_joints:
        p.setJointMotorControl2(aubo, joint_idx, p.POSITION_CONTROL, targetPosition=angle * multiplier, force=10)
...
cube_id = p.loadURDF("figure/cube.urdf", basePosition=[0.2, 0.7, 0.26], 
                     ...
                     useFixedBase=False, globalScaling=0.55, 
...
can_pos = [0.11, 0.53, 0.56]
can_id = p.loadSoftBody(
    "sphere_smooth.obj",
    basePosition=can_pos,
    scale=0.05,
    mass=0.5,
