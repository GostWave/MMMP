...
with open('scene/config/config_1.json', 'r') as f:
    cfg = json.load(f)

# параметры для cube_spacing в config_1.json
# 0.45 - близко
# 1.0 - свободно
# 2.0 - разнесены 
sp = cfg["cube_spacing"]
...
table_configs = [
    {"file": "figure/table_1.urdf", "pos": [ dist_1,  0, 0], "ori": [0, 0, 1.57]},
    {"file": "figure/table_2.urdf", "pos": [ 0,  dist, 0], "ori": [0, 0, 0]},
    {"file": "figure/table_3.urdf", "pos": [-dist_1,  0, 0], "ori": [0, 0, 1.57]},
...
cube_positions = [
    [0, -sp, cfg["cube_z_height"]],
    [ 0, 0, cfg["cube_z_height"]],
    [ 0, sp, cfg["cube_z_height"]]
]
...
wall_3_id = p.loadURDF("figure/wall_scene_2.urdf", [wall_x_3, dist-0.42, wall_z_2], p.getQuaternionFromEuler([0,1.57,0]), useFixedBase=True, flags=p.URDF_USE_SELF_COLLISION | p.URDF_USE_INERTIA_FROM_FILE)
...
wall_1 = p.addUserDebugParameter("Wall 1", -(dist +0.5), (dist - 0.9), -dist)
...
while True:
    ...
    p.resetBasePositionAndOrientation(wall_3_id, [wall_x_3, dist-0.42, w3], p.getQuaternionFromEuler([0, 1.57, 0]))
