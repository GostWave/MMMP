# Кубик двигать (разные скорости) с графиками (Done) 
...
for speed_idx, speed in enumerate(speeds):
    for i in range(len(joint_idx)):
        p.resetJointState(aubo, joint_idx[i], start_pos[i])
    p.resetBasePositionAndOrientation(cube_id, [0.8, 0, 0.35], p.getQuaternionFromEuler([0,0,0]))
    ...
    all_force_x.append(np.array(forces_x))
    all_force_y.append(np.array(forces_y))
    all_force_z.append(np.array(forces_z))
    all_time.append(np.array(timestamps))
...
for i, speed in enumerate(speeds):
    axes[0].plot(all_time[i], all_force_x[i], color=colors[i], linestyle=linestyles[i], linewidth=1.5, label=f'speed={speed}')
    axes[1].plot(all_time[i], all_force_y[i], color=colors[i], linestyle=linestyles[i], linewidth=1.5, label=f'speed={speed}')
    axes[2].plot(all_time[i], all_force_z[i], color=colors[i], linestyle=linestyles[i], linewidth=1.5, label=f'speed={speed}')
...
plt.tight_layout()
plt.savefig('Test3/force_all_speeds.png', dpi=300, bbox_inches='tight')
plt.show()